...
const Navbar = () => {
    ...
    return <Flex bg={'white'} alignItems={'center'} justifyContent={'space-between'} boxShadow={{ "base": 'sm' }} py={{ "base": 2, "md": 4 }} px={{ "base": 2, "md": 20 }}>
        <Box>
            <Button as={Link} to={PATH.HOME} variant={'none'} p={0} fontSize={{base: 'xs', md: 'md'}}>{APP.name}</Button>
        </Box>
        <Flex gap={{ "base": 2, "md": 6 }}>
            <Button as={Link} to={PATH.DASHBOARD} variant={'none'} p={0} fontSize={{base: 'xs', md: 'md'}}>Home</Button>
            <Menu>
